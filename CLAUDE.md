...
### Frontend
`webroot/*.html` pages styled by `webroot/css/shared.css` plus per-page inline `<style>`; shared client JS in `webroot/js/` (`api.js`, `basket.js`, `ui.js`, `header.js`, `i18n.js`, `config.js`). The "basket" collects products into a WhatsApp message instead of a checkout; the WhatsApp number reaches the client via `/api/config`. Styling is **hand-written CSS — no Tailwind or CSS framework**. (A dead Tailwind CDN `<script>` was removed; do not re-add it.)

**The header is one block, repeated verbatim.** Every page (including `404.html`, `pyetje-te-shpeshta.html` and `udhezues/*.html`) carries the identical `<header id="main-header">` + `<div class="mobile-nav">` markup — nav row, mega-menu, search row with `#header-search-suggest`, and the grouped mobile drawer — and calls `initHeader()` from `js/header.js` once. That module owns the drawer, the mega-menu, the search suggestions, the scroll-hide and the current-page highlight (driven by `data-nav` attributes, **not** a hardcoded `class="active"`). Its styling lives in `css/shared.css`, not per-page. When you add a category or touch the header, change the markup in *all* pages plus `header.js` — the pages drifted apart once before and the phone menu ended up different per page. `initHeader({ onSearch })` overrides the default "navigate to `/shop.html?q=`" so `shop.html` can filter in place; that page has no search box of its own.
